...
- **Vulnerability Index** represents how sensitive each province is to U.S. tariffs. Values closer to 1 indicate higher vulnerability.
- **User Upload & Prediction** allows users to run a basic regression using their own dataset.
- **GDP Trend Line** provides historical context on Canada's economic performance.
""")

st.sidebar.markdown("""
**📄 CSV Upload Requirements:**
- Upload a file with only numeric columns.
...
if not canada_gdp_series.empty:
    ...
    fig = px.line(canada_gdp_series, x="date", y="value", title="Canada GDP Over Time (USD)")
